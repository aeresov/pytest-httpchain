...
class ReferenceResolver:
    ...
    def resolve_document(self, data: dict[str, Any], base_path: Path) -> dict[str, Any]:
        ...
        Raises:
            LoaderError: If resolution fails
        """
    ...
    def resolve_file(self, path: Path) -> dict[str, Any]:
        ...
        Raises:
            LoaderError: If the file cannot be loaded or references cannot be resolved
        """
        try:
    ...
    def _resolve_external_ref(
        ...
        try:
            ...
            raise LoaderError(f"Failed to load external reference {file_path}: {e}") from e
        finally:
    ...
    def _navigate_pointer(self, data: Any, pointer: str) -> Any:
        ...
        Raises:
            LoaderError: If pointer is invalid
        """
        if not pointer:
            return data
        ...
        try:
            ...
            raise LoaderError(f"Invalid JSON pointer {pointer}: {e}") from e
    ...
    def _merge_with_siblings(
        ...
        if not isinstance(referenced_data, dict):
            if len(siblings) > 0:
                raise LoaderError("Cannot merge non-dict reference with sibling properties")
            return referenced_data
