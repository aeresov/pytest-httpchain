...
class PathValidator:
    ...
    def validate_ref_path(ref_path: str, base_path: Path, root_path: Path, max_parent_traversal_depth: int) -> Path:
        ...
        Raises:
            LoaderError: If the path is invalid or violates security constraints
        """
        # Resolve the path
        ...
        try:
            ...
            raise LoaderError(f"Reference path '{ref_path}' points outside allowed directory tree") from None
        ...
        if parent_traversals > max_parent_traversal_depth:
            raise LoaderError(f"Reference path '{ref_path}' exceeds maximum parent traversal depth of {max_parent_traversal_depth}")
    ...
    def parse_json_pointer(pointer: str) -> list[str]:
        ...
        Raises:
            LoaderError: If the pointer is invalid
        """
        if not pointer:
            return []

        if not pointer.startswith("/"):
            raise LoaderError(f"Invalid JSON pointer: {pointer} (must start with '/')")
