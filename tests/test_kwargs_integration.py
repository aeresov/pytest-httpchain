...
from pytest_http.models import FunctionCall, Request, Response, Save, Scenario, Stage, Verify
...
def sample_variables() -> dict[str, Any]:
    return {"status": 200, "text": "Hello World", "field1": "slideshow.title", "field2": "slideshow.author", "default": "unknown", "timeout": 5.0}


@pytest.fixture
def sample_kwargs() -> dict[str, Any]:
    return {"expected_status": "$status", "expected_text": "$text", "case_sensitive": True}
...
def test_function_call_model(kwargs: dict[str, Any] | None, expected_kwargs: dict[str, Any] | None) -> None:
    func_call = FunctionCall(function="test_module:test_function", kwargs=kwargs)
...
def test_verify_with_function_call() -> None:
    verify = Verify(
        status=HTTPStatus.OK, functions=["test_module:simple_function", FunctionCall(function="test_module:function_with_kwargs", kwargs={"expected_status": 200, "timeout": 5.0})]
    )

    assert verify.functions is not None
...
def test_save_config_with_function_call() -> None:
    save_config = Save(
        ...
        functions=["test_module:simple_function", FunctionCall(function="test_module:function_with_kwargs", kwargs={"field_path": "data.items", "default_value": "unknown"})],
    )
...
def test_stage_with_function_calls() -> None:
    stage = Stage(
        ...
        response=Response(
            verify=Verify(functions=[FunctionCall(function="test_module:verify_function", kwargs={"expected_text": "Hello", "case_sensitive": False})]),
            save=Save(functions=[FunctionCall(function="test_module:save_function", kwargs={"extract_key": "user_id", "default_value": 0})]),
        ),
    )

    assert stage.response is not None
    assert stage.response.verify is not None
    assert stage.response.verify.functions is not None
    assert stage.response.save is not None
    assert stage.response.save.functions is not None
                ...
                "stages": [
                    {
                        ...
                        "response": {
                            "verify": {"functions": [{"function": "test_module:verify_function", "kwargs": {"expected_status": 200, "timeout": 5.0}}]},
                            "save": {"functions": [{"function": "test_module:save_function", "kwargs": {"extract_key": "user_id", "default_value": 0}}]},
                        },
                    }
                ]
            },
...
def test_scenario_with_function_calls(scenario_data: dict[str, Any], expected_verify: dict[str, Any] | None, expected_save: dict[str, Any] | None) -> None:
    ...
    assert scenario.stages is not None
    ...
    if expected_verify:
        ...
        # This scenario doesn't have any functions
        assert stage.response.verify.functions is None
        assert stage.response.save.functions is None
        ...
        "stages": [
            {
                ...
                "response": {
                    "verify": {"functions": ["test_module:simple_function", {"function": "test_module:function_with_kwargs", "kwargs": {"param": "value"}}]},
                    "save": {"functions": ["test_module:simple_save_function", {"function": "test_module:save_with_kwargs", "kwargs": {"field": "data"}}]},
                },
            }
        ]
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"param1": "value1", "param2": 42, "param3": True}, {"unused": "value"}, {"param1": "value1", "param2": 42, "param3": True}),
        (
        ...
        ({"param1": "$var1", "param2": "static_value"}, {}, {"param1": "$var1", "param2": "static_value"}),
    ],
)
...
def test_substitute_kwargs_variables_nested() -> None:
    kwargs = {"fields": ["$field1", "$field2", "static_field"], "config": {"default_value": "$default", "timeout": "$timeout"}}

    variables = {"field1": "slideshow.title", "field2": "slideshow.author", "default": "unknown", "timeout": 5.0}

    result = substitute_kwargs_variables(kwargs, variables)

    expected = {"fields": ["slideshow.title", "slideshow.author", "static_field"], "config": {"default_value": "unknown", "timeout": 5.0}}

    assert result == expected


def test_substitute_kwargs_variables_complex_json() -> None:
    kwargs = {"config": {"filters": [{"field": "$field1", "value": "$value1"}, {"field": "$field2", "value": "$value2"}], "options": {"timeout": "$timeout", "retries": 3}}}

    variables = {"field1": "title", "value1": "Sample", "field2": "author", "value2": "Yours Truly", "timeout": 5.0}

    result = substitute_kwargs_variables(kwargs, variables)

    expected = {"config": {"filters": [{"field": "title", "value": "Sample"}, {"field": "author", "value": "Yours Truly"}], "options": {"timeout": 5.0, "retries": 3}}}
...
def test_call_function_with_kwargs(mock_response: Mock) -> None:
    def test_function(response: Mock, expected_status: int = 200, expected_text: str = "") -> bool:
        return response.status_code == expected_status and expected_text in response.text
    ...
    with patch("importlib.import_module") as mock_import:
        ...
        result = call_function_with_kwargs("test_module:test_function", mock_response, {"expected_status": 200, "expected_text": "Hello"})
    ...
    with patch("importlib.import_module") as mock_import:
        ...
        result = call_function_with_kwargs("test_module:test_function", mock_response)
