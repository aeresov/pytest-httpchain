...
class TestSSLPathValidators:
    """Test SSL path validation with pathlib.Path."""

    def test_pathlib_path_creation(self):
        """Test valid SSL certificate paths create valid Path objects."""
        valid_paths = [
        ...
        for path_str in valid_paths:
            path = Path(path_str)
            assert str(path) == path_str or str(path) == Path(path_str).as_posix()
...
class TestSSLConfig:
    ...
    def test_ssl_config_verify_path(self):
        ...
        config = SSLConfig(verify=Path("/etc/ssl/certs/ca-bundle.crt"))
        assert config.verify == Path("/etc/ssl/certs/ca-bundle.crt")
    ...
    def test_ssl_config_cert_tuple(self):
        ...
        assert config.cert == (Path("/path/to/client.crt"), Path("/path/to/client.key"))

    def test_ssl_config_combined(self):
        """Test SSL config with both verify and cert."""
        config = SSLConfig(verify=Path("/etc/ssl/certs"), cert=(Path("/path/to/client.crt"), Path("/path/to/client.key")))
        assert config.verify == Path("/etc/ssl/certs")
        assert config.cert == (Path("/path/to/client.crt"), Path("/path/to/client.key"))
    ...
    def test_ssl_config_verify_empty_path(self):
        """Test SSL config with empty verify path - Path('') is valid."""
        config = SSLConfig(verify=Path(""))
        assert config.verify == Path("")

    def test_ssl_config_cert_empty_path(self):
        """Test SSL config with empty cert path - Path('') is valid."""
        config = SSLConfig(cert=Path(""))
        assert config.cert == Path("")

    def test_ssl_config_cert_empty_in_tuple(self):
        """Test SSL config with empty cert path in tuple - Path('') is valid."""
        config = SSLConfig(cert=(Path("/path/to/client.crt"), Path("")))
        assert config.cert == (Path("/path/to/client.crt"), Path(""))

    def test_ssl_config_cert_invalid_tuple_length(self):
        """Test SSL config with invalid tuple length."""
        with pytest.raises(ValidationError):
            SSLConfig(cert=(Path("/path/to/client.crt"),))  # Only one element

        with pytest.raises(ValidationError):
            SSLConfig(cert=(Path("/path/to/client.crt"), Path("/path/to/client.key"), Path("/extra")))  # Three elements
...
class TestScenarioSSL:
    ...
    def test_scenario_with_ssl(self):
        ...
        scenario = Scenario(ssl=SSLConfig(verify=Path("/etc/ssl/certs"), cert=(Path("/path/to/client.crt"), Path("/path/to/client.key"))))
        assert scenario.ssl.verify == Path("/etc/ssl/certs")
        assert scenario.ssl.cert == (Path("/path/to/client.crt"), Path("/path/to/client.key"))
    ...
    def test_scenario_ssl_from_dict(self):
        ...
        assert scenario.ssl.cert == Path("/path/to/client.pem")
...
class TestRequestSSL:
    ...
    def test_request_with_ssl(self):
        ...
        request = Request(url="https://api.example.com/test", ssl=SSLConfig(verify=False, cert=Path("/path/to/client.pem")))
        assert request.ssl.verify is False
        assert request.ssl.cert == Path("/path/to/client.pem")
    ...
    def test_request_ssl_from_dict(self):
        ...
        assert request.ssl.cert == (Path("/path/to/client.crt"), Path("/path/to/client.key"))
...
class TestSSLSerialization:
    ...
    def test_request_ssl_model_dump(self):
        ...
        request = Request(url="https://api.example.com/test", ssl=SSLConfig(verify=True, cert=(Path("/client.crt"), Path("/client.key"))))
    ...
    def test_roundtrip_serialization(self):
        ...
        assert restored_scenario.ssl.verify == Path("/custom/ca-bundle.crt")
        assert restored_scenario.ssl.cert == (Path("/path/to/client.crt"), Path("/path/to/client.key"))
