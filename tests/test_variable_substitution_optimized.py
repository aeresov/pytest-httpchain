...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"name": "auth_stage", "url": "https://api.example.com/users/$user_id/posts", "headers": {"Authorization": "Bearer $token"}, "params": {"limit": "$page_size"}},
            {"user_id": 456, "token": "abc123", "page_size": 10},
            {"name": "auth_stage", "url": "https://api.example.com/users/456/posts", "headers": {"Authorization": "Bearer abc123"}, "params": {"limit": 10}},
            "complex_stage",
        ),
        # No substitutions
        (
...
@pytest.mark.parametrize(
    ...
    [
        ("substitute_variables", '{"data": "$invalid"}', {"invalid": object()}, "Failed to substitute variables"),
        ("substitute_stage_variables", {"data": "$invalid"}, {"invalid": object()}, "Failed to substitute variables in stage"),
    ],
)
